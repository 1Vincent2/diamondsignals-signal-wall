...
The `/scout/<player_id>/` path is an intentional supporting route layer for
Player Scout / Dossiers. It is generated by `dashboard/build_dashboard.py` from
`dist/dossier_canon.json` and materialized as `dist/scout/<player_id>/index.html`.

This route is not a standalone report surface. It does not require its own
`dist/status/*.json` contract entry. Its route contract is instead declared as
a supporting route under the Signal Wall inventory record, because Signal Wall
owns the canonical dossier generation pass.

Audit interpretation:
- `/scout/<player_id>/` links are valid only when the target player exists in
  `dist/dossier_canon.json` and a generated dossier page is present.
- Signal pages may link to `/scout/<player_id>/` only through canonical
  `player_id` resolution.
- Refinement-closure audits must treat this route as an intentional supporting
  route layer, not as an orphan public report surface.
