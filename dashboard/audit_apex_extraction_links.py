...
URL = f"{BASE}/apex-extraction/?v={int(datetime.now().timestamp())}"

html = urllib.request.urlopen(URL, timeout=30).read().decode("utf-8", errors="replace")

ids = re.findall(r'(?:data-profile-url|href)="/scout/([0-9]+)/"', html)
ids = list(dict.fromkeys(ids))

print("--- Apex Extraction parsed JSON audit-link hardening check ---")
print("apex_extraction_url:", URL)
